# Don't rename the following section, its contents are auto-inserted based on the name (see scripts/pontoon-languages-to-ftl.js)
# [Languages]


## Languages

aa = Afar
aat = Arvanitisch
ab = Abchazisch
abb = Bankon
ace = Atjehs
ady = Adygees
af = Afrikaans
aln = Gegisch
am = Amhaars
an = Aragonees
ar = Arabisch
arn = Mapudungun
as = Assamees
ast = Asturisch
az = Azeri
azz = Hoogland-puebla-nahuatl
ba = Basjkiers
bal = Balochi
bas = Basaa
bax = Bamun
bba = Baatonum
bbj = Ghomala
bbl = Batsisch
bci = Baoule
be = Wit-Russisch
beb = Bamvele
bew = Betawi
bfd = Bafut
bft = Balti
bg = Bulgaars
bgp = Oost-Beloetsji
bkm = Kom
bm = Bambara
bn = Bengaals
bnm = Batanga
bnn = Bunun
bo = Tibetaans
br = Bretons
bri = Mokpwe
bs = Bosnisch
bsh = Katë
bsk = Brushaski
bum = Bulu
bxr = Boerjatisch
byv = Medumba
ca = Catalaans
cak = Kaqchikel
cdo = Oosterse Min
ckb = Centraal-Koerdisch
cnh = Hakha Chin
co = Corsicaans
crh = Krim-Tataars
cs = Tsjechisch
cut = Teutila-Cuicatec
cux = Tepeuxila-Cuicatec
cv = Chuvash
cy = Welsh
da = Deens
dag = Dagbani
dar = Dargwa
dav = Kidaw'ida
de = Duits
dmk = Dawoodi
dml = Dameli
dru = Rukai
dsb = Nedersorbisch
dua = Duala
dv = Divehi
dyu = Dioula
ebr = Ebrie
ee = Ewe
el = Grieks
en = Engels
eo = Esperanto
es = Spaans
esu = Centraal-Alaska Yup’ik
et = Estisch
eto = Eton
eu = Baskisch
ewo = Ewondo
fa = Perzisch
fan = Fang
ff = Fula
fi = Fins
fo = Faeröers
fr = Frans
fub = Adamawa Fulfulde
fue = Borgu Fulfulde
fuf = Guinee-Pular
fy-NL = Fries
ga-IE = Iers
gju = Gojiri
gl = Galicisch
gn = Guaraní
gom = Goanees Konkani
gos = Gronings
gsw = Elzassisch
gu-IN = Gujarati
guc = Wayuunaiki
gv = Manx-Gaelisch
gwc = Gawri
gwt = Gawar-Bati
gya = Noordwest-GBaya
ha = Hausa
hac = Našinski
hch = Huichol
he = Hebreeuws
hi = Hindi
hil = Hiligaynon
hno = Noord-Hindko
hr = Kroatisch
hsb = Oppersorbisch
ht = Haïtiaans
hu = Hongaars
hy-AM = Armeens
hyw = West-Armeens
ia = Interlingua
ibb = Ibibio
id = Indonesisch
ie = Interlingue
ig = Igbo
ipk = Inupiak
is = IJslands
it = Italiaans
izh = Izjorisch
ja = Japans
jbo = Lojban
jqr = Jaqaru
jv = Javaans
ka = Georgisch
kaa = Karakalpaks
kab = Kabylisch
kbd = Kabardisch
kcn = Nubi
khw = Khowar
ki = Kikuyu
kk = Kazachs
kln = Kalenjin
kls = Kalasha
km = Khmer
kmr = Kurmanci
kn = Kannada
knn = Konkani (Devanagari)
ko = Koreaans
kpv = Zurjeens
ksf = Bafia
kw = Cornish
ky = Kirgizisch
lb = Luxemburgs
leu = Kara
lg = Luganda
lij = Ligurisch
lke = Soga
ln = Lingala
lo = Laotiaans
lss = Lasi
lt = Litouws
ltg = Letgaals
lth = Thur
luo = Dholuo
lv = Lets
lzz = Laz
mai = Maithili
mau = Huautla-Mazateeks
mbo = Mbo
mdf = Moksja
meh = Mixteco-Yucuhiti
mg = Plateaumalagasi
mhr = Weide-Mari
mk = Macedonisch
ml = Malayalam
mmc = Michoacán-Mazahua
mn = Mongools
mni = Meetei Lono
mos = More
mqh = Tlazoyaltepec-Mixteeks
mr = Marathi
mrj = Berg-Mari
ms = Maleis
mt = Maltees
mvy = Indus Kohistani
mxu = Mada
my = Burmees
myv = Erzja
nan-tw = Taiwanees (Minnan)
nb-NO = Noors Bokmål
ncx = Centraal-Puebla-Nahuatl
nd = IsiNdebele (Noordelijk)
ne-NP = Nepalees
nhe = Oostelijk Huasteca Nahuatl
nhi = Westelijk Sierra Puebla Nahuatl
nia = Nias
nl = Nederlands
nn-NO = Noors Nynorsk
nnh = Ngiembon
nqo = NKo
nr = IsiNdebele (Zuidelijk)
nso = Noord-Sotho
ny = Chinyanja
nyn = Nyankole
oc = Occitaans
om = Afaan Oromo
or = Odia
oru = Ormuri
os = Ossetisch
pa-IN = Punjabi
pap = Papiaments
pap-AW = Papiaments (Aruba)
pcm = Nigeriaans pidgin-Engels
phl = Palula
pl = Pools
plk = Kohistani Shina
ps = Pashtu
pt = Portugees
pua = Purepecha van de Westelijke Hooglanden
pwn = Paiwaans
quc = K'iche'
qup = Zuid-Pastaza Quechua
qux = Quechua-Yauyos
quy = Quechua Chanka
qva = Quechua Ambo-Pasco
qvi = Kichwa
qvl = Quechua Cajatambo
qwa = Quechua Corongo Ancash
qws = Quechua Sihuas Ancash
qxa = Quechua Chiquián
qxp = Puno Quechua
qxt = Quechua Pasco Santa Ana de Tusi
qxu = Quechua Arequipa-La Unión
qxw = Quechua Jauja Wanka
rif = Tarifit
rm-sursilv = Reto-Romaans Surselvisch
rm-vallader = Reto-Romaans Vallader
ro = Roemeens
ru = Russisch
ruc = Ruuli
rw = Kinyarwanda
sah = Jakoets
sat = Santali (Ol Chiki)
sc = Sardijns
scl = Shina
scn = Siciliaans
sco = Schots
sd = Sindhi
sdh = Zuid-Koerdisch
sei = Seri
shi = Tashelhiyt (Shilha)
shn = Shan
si = Singalees
sk = Slowaaks
skr = Saraiki
sl = Sloveens
snk = Soninke
so = Somalisch
sq = Albanees
sr = Servisch
ss = Siswati
st = Zuid-Sotho
sv-SE = Zweeds
sva = Svanetisch
sw = Swahili
syr = Syrisch
szy = Sakizaya
ta = Tamil
tay = Atayal
te = Telugu
teg = Teke
tg = Tadzjieks
th = Thai
ti = Tigrinya
tig = Tigre
tk = Turkmeens
tl = Tagalog
tn = Tswana
tob = Toba Qom
tok = Toki Pona
top = Papantla Totonac
tr = Turks
trv = Taroko
trw = Torwali
ts = Tsonga
tt = Tataars
tui = Tupuri
tvu = Tunen
tw = Twi
ty = Tahitiaans
tyv = Toevaans
uby = Oebychs
udm = Oedmoerts
ug = Oeigoers
uk = Oekraïens
ukv = Kuku
ur = Urdu
ush = Ushoji
uz = Oezbeeks
var = Guarijio
ve = Venda
vec = Venetiaans
vi = Vietnamees
vmw = Emakhuwa
vot = Wotisch
wbl = Wachi
wep = Westfaals
wes = Kameroen Pidgin
wo = Wolof
xh = Xhosa
xhe = Khetrani
xka = Kalkoti
xmf = Mingreels
yaq = Yaqui
yi = Jiddisch
yo = Yoruba
yue = Kantonees
zgh = Tamazight
zh-CN = Chinees (China)
zh-HK = Chinees (Hongkong)
zh-TW = Chinees (Taiwan)
zoc = Copainalá Zoque
zu = Zoeloe
zza = Zazaki

# [/]


## Layout

profile = Profiel
help = Help
contact = Contact
privacy = Privacy
terms = Voorwaarden
cookies = Cookies
faq = FAQ
documentation = Documentatie
content-license-text = Inhoud is beschikbaar onder een <licenseLink>Creative Commons-licentie</licenseLink>
share-title = Help ons anderen te vinden die hun stem willen doneren!
share-text = Help machines te leren hoe echte mensen spreken, doneer uw stem op { $link }
link-copied = Koppeling gekopieerd
back-top = Naar boven
logout = Afmelden
donate = Doneren
stayintouch = Bij Mozilla bouwen we een gemeenschap rond spraaktechnologie. We blijven graag met u in contact met updates, nieuwe gegevensbronnen, en om te horen hoe u deze gegevens gebruikt.
privacy-info = We beloven uw gegevens met zorg te behandelen. Lees meer in onze <privacyLink>Privacyverklaring</privacyLink>.
return-to-cv = Terug naar website
email-input =
    .label = E-mailadres
loading = Laden…
email-opt-in-info = Ik wil graag e-mailberichten zoals herinneringen aan doelstellingen, mijn voortgangsupdates en nieuwsbrieven ontvangen.
email-opt-in-info-title = Meld u aan voor de mailinglijst
email-opt-in-info-sub-with-challenge = Ontvang e-mailberichten zoals herinneringen aan uitdagingen en doelstellingen, voortgangsupdates en nieuwsbrieven.
email-opt-in-privacy-v2 = Door u in te schrijven voor het ontvangen van e-mailberichten, verklaart u dat u akkoord gaat met het verwerken van deze informatie door Mozilla in overeenstemming met Mozilla’s <privacyLink>Privacybeleid</privacyLink>.
indicates-required = * Geeft verplicht veld aan
not-available-abbreviation = NB

## Donate banner

# Text wrapped in <mark></mark> will have a white background.
donate-banner-cta = <mark>We hebben</mark><br/><mark>uw hulp nodig!</mark>
# Text wrapped in <strong></strong> will be rendered on bold font
donate-banner-cta-explanation = Het kost bijna een miljoen dollar per jaar om de gegevenssets te hosten en het platform te verbeteren voor de meer dan 100 taalgemeenschappen die afhankelijk zijn van wat we doen. <strong>Als u open, inclusieve gegevens waardeert – doneer dan vandaag nog!</strong>
# Text wrapped in <mark></mark> will have a white background.
languages-donate-banner-cta = <mark>Gebruikt u dit project voor</mark><br/><mark>uw onderzoek?</mark>
languages-donate-banner-cta-explanation = Dit project wordt gefinancierd door donaties en subsidies! We werken graag samen met academici, onderzoekers van het maatschappelijk middenveld en de industrie. Dit platform is gratis te gebruiken, maar bijdragen aan de platform- en hostingkosten via subsidievoorstellen helpt echt.
localization-select =
    .label = Kies uw variant
